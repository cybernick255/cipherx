...
struct SecKeyStore {
    ...
    func roundTrip<T: SecKeyConvertible>(_ key: T) throws -> T {
        ...
        let label: String = Constants().labelSecKeyPrivateKey
    ...
    func hasKey<T: SecKeyConvertible>(label: String, as type: T.Type) -> Bool
    {
        do
        {
            let key: T? = try readKey(label: Constants().labelSecKeyPrivateKey)
            
            return key != nil
        }
        catch
        {
            print("Keychain error while checking key: \(error)")
